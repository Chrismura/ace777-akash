# SPEC v2 — BUDGET : GRATUITS DYNAMIQUES (jamais figés dans le code)

## Statut

v1 du livrable budget validée (JUGE/GEMINI/ULTRA GO, DEEPSEEK hors périmètre).
Nouvelle décision Christophe qui PRIME sur la v1 :

> « Gratuit jamais coupé, à voir : car un autre gratuit dans les bookmarks meilleur
> ou plus rapide on change immédiatement. Dans la tempête on s'arrange au mieux.
> Valeur fixe on coule. »

**Principe** : la liste des providers gratuits est une DONNÉE VIVANTE, déclarée dans
`providers.json` (champ `free`), PAS une constante dans le code. La rotation choisit
le meilleur gratuit au moment T. En tempête, on s'arrange au mieux (toutes les
options dispo). Aucune valeur fixe nulle part.

## RÈGLES ABSOLUES

1. Python 3.9 stdlib, pas de dépendance externe.
2. `typing.Optional`, jamais `str | None`.
3. Non fatal, UTF-8, ne pas toucher au moteur ACE.
4. **Ne pas casser le comportement nominal du hub** (tant que le budget n'est pas
   atteint, comportement identique).

---

## MODIF A — Déclarer le champ `free` dans `providers.json`

Ajouter `"free": true` aux providers qui ne coûtent rien (gratuits réels) :

- `gemini` → free: true
- `openrouter-free` → free: true
- `openrouter-ultra` → free: true
- `openrouter-juge` → free: true
- `nvidia` → free: true
- `inferx-coder` → free: true
- `puter-grok` → free: true
- `qwen-local` → free: true (local, jamais compté)
- tous les autres → free: false (ou absent)

C'est le seul endroit où la liste des gratuits vit. Changer un provider en gratuit =
modifier `providers.json`, jamais le code.

## MODIF B — `budget_hub.py` : lire les gratuits depuis providers.json

- **B1.** Supprimer la constante `GRATUITS = {...}` codée en dur.
- **B2.** Ajouter une fonction `gratuits_actifs()` qui lit `providers.json` et
  retourne les ids avec `free == true` (et `enabled` ou référencés dans routing,
  comme `providers_actifs()`).
- **B3.** `calculer_budget_journalier()` reçoit la liste des gratuits (paramètre ou
  appelle `gratuits_actifs()`) et retourne `gratuits` = cette liste dynamique.
- **B4.** `main()` affiche « Gratuits proteges (dynamiques) : ... » depuis la liste
  lue dans providers.json.

## MODIF C — `hub_prise_ia.py` : les gratuits viennent de providers.json

- **C1.** Supprimer la constante `GRATUITS` du bloc d'intégration.
- **C2.** Construire la liste des gratuits depuis les providers chargés :
  ```python
  gratuits = {p.get("id") for p in providers if p.get("free")}
  ```
- **C3.** La logique de bascule utilise cette liste dynamique : si le budget calme
  est atteint, les gratuits restent éligibles (bascule même famille), les payants
  sont écartés — en préservant l'ordre de `target_ids`.
- **C4.** En TEMPÊTE, « on s'arrange au mieux » : aucune restriction gratuits/payants
  pour les tâches prioritaires — toutes les options de `target_ids` restent
  éligibles, plus la réserve storm si nécessaire. La tempête ne voit JAMAIS de coupure.
- **C5.** Message : « Budget calme atteint → bascule famille (gratuits) » quand un
  gratuit est dispo ; « plus de gratuit disponible » seulement si aucun gratuit.

## MODIF D — Tests (`~/prise-ia/test_budget_storm.py`)

Mettre à jour les tests :
- T3 devient : avec un provider marqué `free: true` dans providers.json (simulé en
  /tmp), budget atteint → il reste éligible ; un provider sans `free` → écarté.
- T6 : en tempête + budget calme atteint → aucune coupure (toutes les options dispo,
  réserve utilisée si nécessaire).

## CONTRAT DE SORTIE

- Le bloc `providers.json` à modifier (avec `free` ajouté).
- `budget_hub.py` entier (gratuits dynamiques).
- Le bloc d'intégration `hub_prise_ia.py` (gratuits dynamiques + tempête sans
  coupure).
- `test_budget_storm.py` entier (tests mis à jour).
- Zéro placeholder, syntaxe Python 3.9 valide, commentaires en français, non fatal.

## FICHIERS CONCERNÉS

- `~/prise-ia/providers.json` (MODIF A : champ free)
- `~/prise-ia/budget_hub.py` (MODIF B)
- `~/prise-ia/hub_prise_ia.py` (MODIF C)
- `~/prise-ia/test_budget_storm.py` (MODIF D)


---

## ANNEXE — CODE v1 (base, à dynamiser)
