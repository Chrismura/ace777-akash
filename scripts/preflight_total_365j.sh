...
# Même logique stricte que verif_sterilite (pas de bare "ace777" → faux positifs chemin projet)
if pgrep -fl 'ace777_launch_v85|launch_vide_froid_4h_binance|launch_test_master_base_v8_6_fortress|watchdog_ace777|genesis_manifest\.txt|tail -n 0 -F runs/|tail -F runs/\.NUAGE|/tmp/launch_vide_froid_4h_binance_NUAGE' 2>/dev/null | grep -vi ollama | grep -vi 'verif_sterilite\|preflight_total\|GO_USINE' | grep -q .; then
