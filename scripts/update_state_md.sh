...
# B1 — Journal Engle (lecture seule, hors moteur)
if [ -f "${_root}/scripts/engle_journal.rb" ]; then
  ruby "${_root}/scripts/engle_journal.rb" || echo "ENGLE_JOURNAL: skip (non bloquant)"
fi
