# Réponse codeur — SPEC_VIGIE_MEMPOOL_2026-08-16.md (provider Google Gemini, 13.0s)

Reçu, chef. En tant que codeur senior ACE777, je livre ici le script strict, robuste, en stdlib pure, intégrant le correctif anti-faux-positifs par carnet d'historique glissant (`mempool_vus.jsonl`), le kill-switch impératif et l'écriture atomique.

Voici le fichier complet `Index_Maison/scripts/detecter_bloc_privatise.py`.

### `Index_Maison/scripts/detecter_bloc_privatise.py`
...
def get_latest_block_info():
    """Récupère le hash du dernier bloc et ses txids."""
    tip_hash = http_get_json("https://mempool.space/api/blocks/tip/hash")
    if not isinstance(tip_hash, str):
        return None, []
...
### NOTES (Choix faits, points d'attention, doctrine)

1. **Correction anti-faux-positifs (P0)** : 
   - Au lieu de comparer un instant T0 et T0+2s, le script maintient un journal append-only (`data/mempool_vus.jsonl`) des transactions observées à chaque exécution/snapshot.
   - Une fenêtre glissante de **60 minutes** (`WINDOW_MINUTES = 60`) est appliquée lors du chargement de l'historique. Si une transaction d'un bloc miné a été vue au moins une fois dans notre carnet au cours de la dernière heure, elle est considérée comme normale (entrée dans la mempool avant d'être minée). Si elle n'y est **jamais** apparue, c'est une **tx fantôme / bloc privatisé**.
2. **Pré-filtre API (P1 - Free Tier)** : 
   - Pour respecter les quotas de mempool.space sans clé API, le script ne fait **pas** d'appel unitaire `/api/tx/{txid}` pour chaque transaction du bloc.
   - Il ne creuse le détail des transactions cachées qu'en cas de dépassement d'un seuil (`ALERT_TX_THRESHOLD = 5`). En dessous, le volume est comptabilisé à 0 en mode observation rapide, préservant totalement la bande passante et évitant le rate-limiting.
3. **Mode Observation & Kill-Switch** : 
   - `mode = "observation"` par défaut. `alerte_potentielle.emise = false` de manière permanente et codée en dur.
   - Vérification systématique de `Index_Maison/strategie/STOP`, `Index_Maison/STOP_ALL` et du stop global avant chaque écriture ou appel réseau.
   - Écriture atomique systématique (`tempfile.mkstemp` + `os.replace`) pour tous les fichiers de données JSON.
