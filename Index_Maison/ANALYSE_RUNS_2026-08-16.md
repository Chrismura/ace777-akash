...
## 7. Arrêt du run 96h — 17/08 (décision Christophe)

Le run 96h (setup C, champion `8bce77b1`) est **arrêté** le 17/08 au matin :
- plist `com.ace777.run-vortex-96h` bootoutée (KeepAlive retiré → plus de relance auto)
- `STOP` / `STOP_ALPHA` / `STOP_BETA` posés (relance bloquée tant qu'ils existent — `GO_VORTEX_V2.sh` les supprime au lancement)
- Zéro process ACE restant (vérifié)

**Motif** (Christophe) : le setup C « FIX-LAST-LOSS » n'est pas le bon — on reprend les **2 setups à tester** qui changent le revenge.

## 8. Les 2 setups à tester (reprise 17/08)

| Setup | Changement revenge | Bascule exacte |
|---|---|---|
| **A. Rollback complet** (avant fix heartbeat 15/08 15:30) | revenge armé en permanence (~58→89%) | `cp genesis_manifest.txt.BAK_avant_fix_heartbeat_20260815-152847 LE_VRAI_CHAMPION_CERTIFIE_37FCA367.txt` → md5 `fe2a7bcc…` |
| **B. TTL revenge 20→120s** (fix 15/08 conservé) | fenêtre revenge élargie, `stale_state` parasite conservé | `export DUO_EVENT_TTL_SEC=120` au lancement (défaut 20 dans le manifest, ligne 301) — **aucune modif code** |

**Vérifié 17/08** : backup A présent et md5 `fe2a7bcc9dc1f31bd524ffc433f9186d` ✅ · `DUO_EVENT_TTL_SEC` lisible via env dans le manifest (ligne 301 : `${DUO_EVENT_TTL_SEC:-20}`) ✅

**À décider** : quel setup tester en premier (A ou B) → puis `./GO_VORTEX_V2.sh <durée>` (ex. `04:00:00` pour 4h).
