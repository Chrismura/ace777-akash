...
| S-10 | **Frais de plateforme vs PnL NET (CRITIQUE — trouvé 19/08)** : le bot affichait un PnL BRUT (+14 session) alors que le NET est −278 (frais Binance −293 aujourd'hui ≈ 88 % du trou). Le moteur paper n'ôte PAS les frais de son PnL → il décide sur un signal faux. **Fait** : `comboPnlNet` + `comboFees` dans mission.json, bulle FRAIS en bas de la carte BETA, plist `com.ace777.fees` (5 min). **Reste** : (a) filtre d'entrée « edge > coût des frais », (b) ordres maker/post-only (frais ÷2 à 4), (c) corriger le PnL interne de genesis (sealed), (d) barre de qualité = point 4 de Christophe (intégré après). | 🟠 en cours |
