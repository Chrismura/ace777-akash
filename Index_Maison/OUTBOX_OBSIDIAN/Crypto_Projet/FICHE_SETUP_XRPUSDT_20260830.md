# 🎯 FICHE SET-UP INDIVIDUEL — XRPUSDT (XRP (Ripple)) — 30/08/2026

> **Système unique, set-up propre à CET actif** (doctrine : même système, pas même set-up).
> Profil automatique : `hulk-mexc/runs/profils_actifs/PROFIL_XRPUSDT.md` (2290 points).
> Suivi quotidien : `hulk-mexc/runs/SUIVI_SETUP_XRPUSDT.md` (mesure jour par jour, plist 16:35 locale).
...
|---|---|---|
| **Range total** | 182.9% | Amplitude sur la fenêtre |
| **Creux intraday** | **18h UTC** | Fenêtre d'entrée : **17h / 18h / 19h** |
...
| **Volatilité (dd15 moy)** | 17.55% | ÉLEVÉ — dd15 moyen 18% (volatile, stops à respecter) |
| **Mur bid max** | 606420.0$ (spoof 4.72%) | Mur = INFO, spoof = TENSION |
| **Poussière moyenne** | 14.6% | Déclencheur entrée : < 15% |
| **Corrélation BTC** | 0.95 | fortement corrélé BTC (+0.95) → suit le marché |
...
1. **Fenêtre** : uniquement **17h / 18h / 19h UTC** (le creux de CET actif).
2. **Déclencheur** : poussière < 15% + mur testé qui tient.
3. **NON — actif de marché (filtre macro >1.5% BTC/ETH indispensable)** — fortement corrélé BTC (+0.95) → suit le marché.
...
- **ÉLEVÉ — dd15 moyen 18% (volatile, stops à respecter)**.
...
## ⏱️ ÉTAT ACTUEL
- **XRPUSDT est suivie en observation** (doctrine : tous les actifs sous observation).
...
## Archives
- Profil : `hulk-mexc/runs/profils_actifs/PROFIL_XRPUSDT.md`
- Suivi : `hulk-mexc/runs/SUIVI_SETUP_XRPUSDT.jsonl` + `.md`
