...
- **Horloge live — TRANCHÉE EN R36 (Gemini, 07/09)** : **l'horloge NE repart PAS du 05/09** — la coupure batterie fait partie de la réalité opérationnelle à tester (biais de confirmation de l'effacer) ; le compteur global reste ancré au J0 du 02/09 (verdict initial 16/09) ; S2 est jugé avec son artefact documenté (trou 102 min → 2 pertes invalidées, −55,16 $ ; hors artefact S2 ≈ +30,41). Les segments restent analysés séparément, jamais mélangés. (Note : la proposition Buffy « verdict 19/09 » est abandonnée sur avis juge — cf. `CONFRONTATION_R36_20260907.md` §C.)
- **Audit des pires trades (07/09, règles figées)** : 0 trou de données dans S1/S3 et les 4 fenêtres → les catastrophes (−99,87 MARS, −51,39 NUAGE, −41,91 S1) sont du VRAI MARCHÉ, toutes en cap-2h à 120 min. Seul S2 contaminé (trou 102 min). Le filtre de régime n'aurait bloqué AUCUNE catastrophe (ratios d'entrée 0,2×–1,1×) → l'orage naît PENDANT la détention → l'assurance de queue = stop de structure (V3), pas un filtre d'entrée. Cf. `RAPPORT_L2_ET_K3_20260907.md` PARTIES 6-7.
- **Filtre de régime : CADUC (07/09)** — la v1 du replay avait un artefact (base=None pendant les 12 premières heures → entrées bloquées par absence de données, pas par le filtre). En v2 corrigée (pré-data 25 h) : 0 entrée refusée, écart +0,00 partout. La gate H est déjà un filtre de régime de facto. Le dilemme « strict vs risque-ajusté » posé à la famille est évaporé.
