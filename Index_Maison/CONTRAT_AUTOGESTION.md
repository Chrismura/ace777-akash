...
## 1sexies. LA RÈGLE DU LOT — audits regroupés, zéro gaspillage (gravée 10/08/2026)

> Déclaration de Christophe : *« ici on traite que des solutions durables. C'est la règle du contrat, c'est clair — on ne fait pas du bricolage. »*

**Principe :** un audit famille (loi 1quater) coûte du crédit cloud. Le soumettre à chaque étape est un gaspillage. Les audits sont **regroupés par lot** — un seul audit diff par chantier (ou par jour max), pas un par étape.

| AVANT (bricolage) | APRÈS (durable) |
|---|---|
| Chaque commit → audit famille (GEMINI + JUGE) | **Un audit par chantier terminé** (ou par jour, le premier arrivé) |
| 10 audits pour 10 étapes = 20 appels cloud | 1 audit pour 10 étapes = 2 appels cloud |
| On valide le détail avant la fin | On valide le résultat final, pas le chemin |

**Règles concrètes :**
1. **On ne soumet pas chaque changement un par un** — on accumule les modifications (branch git) et on soumet **un seul audit diff** quand le chantier est fini
2. **Maximum 1 audit famille par jour** — sauf urgence (escalade ROUGE, panne critique)
3. **L'audit porte sur le diff complet** (git diff entre deux tags) — pas de résumé, pas de sélection
4. **Le codeur du hub** (task code.ia) n'est pas soumis à la règle du lot — il peut être consulté à chaque étape (c'est un outil, pas un auditeur)
5. **Dérogation possible** : un chantier sensible (qui touche au moteur, à l'argent, à la sécurité) peut être audité étape par étape — mais c'est l'exception qui se justifie, pas le défaut

**Ce que ça change pour la loi 1quater :**
- L'article 1quater reste en vigueur : **tout changement est audité par un tiers de famille différente**
- Mais l'audit se fait **en lot**, pas par étape
- Le verdict final est le même, mais avec 10× moins d'appels cloud

**Ce que ça change pour la loi 1quinquies :**
- 1quinquies (le chief orchestre, le hub code) reste inchangé
- La règle du lot s'applique APRÈS le code, au moment de l'audit

**Preuve d'application :** le 10/08/2026, 42 appels cloud dont 26 audits (62%). Avec la règle du lot, le même chantier aurait consommé ~5 appels d'audit (1 audit diff final) au lieu de 26. Économie : **80% du crédit audit**.

---

> **Règle validée par Christophe le 10/08/2026.** Gravée dans le contrat, applicable immédiatement.
