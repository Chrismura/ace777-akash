...
# == ANTI-TEMPETE PAR TACHE (16/08, methode point final) ==
# Une tache qui echoue 3 fois en 10 min (429/502 partout) est mise en pause 5 min :
# plus jamais une tache fanatique (ex. analyste via 13 vigies) ne sature le hub.
_TASK_PAUSE = {}  # {task: {"until": ts, "echecs": [timestamps]}}
TASK_PAUSE_SEUIL = 3        # 3 echecs
TASK_PAUSE_FENETRE_S = 600  # sur 10 min
TASK_PAUSE_DUREE_S = 300    # -> pause 5 min


def _tache_en_pause(task):
    if not task:
        return False
    p = _TASK_PAUSE.get(task)
    if not p:
        return False
    if time.time() >= p["until"]:
        del _TASK_PAUSE[task]
        return False
    return True


def _tache_echec(task):
    if not task:
        return
    now = time.time()
    p = _TASK_PAUSE.setdefault(task, {"until": 0, "echecs": []})
    p["echecs"] = [t for t in p["echecs"] if now - t < TASK_PAUSE_FENETRE_S]
    p["echecs"].append(now)
    if len(p["echecs"]) >= TASK_PAUSE_SEUIL:
        p["until"] = now + TASK_PAUSE_DUREE_S
        p["echecs"] = []
        log_event("antitempete", "Tache en pause 5 min (3 echecs): " + str(task), "")
...
# Seuil minimal d'une vraie alerte prix pour ouvrir le mode tempete (23/08).
# La vigie ecrit alarme.json des 0.5% en 60s / volume x3 (cooldown 5 min) : en
# marché normal c'est quasi permanent -> l'ancien test (fichier < 1h + type
# present) laissait le mode tempete ACTIF en permanence et desactivait toutes
# les coupes de budget (ex. 2948 appels cloud le 22/08, dont 2607 sur Mistral
# payant via le filet universel). Seule une vraie secousse (>= 1%) ouvre la
# reserve storm ; les alarmes news ne sont jamais une tempete.
ALARME_TEMPETE_MIN_VARIATION_PCT = 1.0
...
def _tail_jsonl(path: str, n: int) -> list:
    """Lit les n dernières lignes d'un JSONL par la fin (lecture bornée, sans tout charger).
    Audit famille 6 : remplace readlines()[-n:] qui lisait le fichier entier en mémoire."""
    rows = []
    ...
    try:
        with open(path, "rb") as f:
            ...
            while pos > 0 and len(rows) < n:
                ...
                if len(data) > 262144:  # buffer borné (256 Ko max)
                    data = data[-262144:]
...
def _raw_call(prov, messages, temperature, max_tokens, timeout):
    ...
    # FIX 17/08 (Groq 403 error code 1010) : Groq est derriere Cloudflare
    # anti-bot — il bloque le User-Agent par defaut de Python (Python-urllib/3.x).
    # Un User-Agent navigateur passe partout (teste : curl marche, urllib non).
    ...
    try:
        ...
        if g and isinstance(g, dict):
            ...
            if zone in ("ROUGE", "PRENDS_LA_PERTE"):
                return True
        try:
            # FIX 23/08 : une alarme n'ouvre le mode tempete que si c'est une
            # VRAIE secousse de prix (>= ALARME_TEMPETE_MIN_VARIATION_PCT). Les
            # alarmes benignes (0.5%/60s, volume x3, news) ne desactivent plus
            # les coupes de budget qui protegent les credits.
            ...
            if os.path.exists(alarme_path) and (time.time() - os.path.getmtime(alarme_path)) < 3600:
                with open(alarme_path, encoding="utf-8") as f:
                    a = json.load(f)
                if isinstance(a, dict) and a.get("type") == "prix":
                    variation = a.get("variation_pct")
                    try:
                        variation = abs(float(variation))
                    except (TypeError, ValueError):
                        variation = 0.0
                    if variation >= ALARME_TEMPETE_MIN_VARIATION_PCT:
                        return True
        except Exception:
            pass
        try:
...
def call_provider(prov, messages, temperature, max_tokens, timeout_budget=None):
    ...
    try:
        ...
        if e.code == 429:
            # 16/08 (Regle 2) : quota/rate-limit -> bascule IMMEDIATE + pause 60s.
            # Le quota se reinitialise pendant qu'on utilise les autres providers.
            log_event("ratelimit", "429 " + prov.get("name", prov.get("id", "?")) + " -> pause 60s + bascule", "")
            with _blacklock:
                _blacklist[prov["id"]] = {"until": time.time() + 60, "level": 1}
            raise
...
def chat_completions(payload):
    ...
    elif isinstance(only_model, str) and only_model and only_model not in ("", "auto"):
    ...
    if target_ids:
        ...
        for tid in target_ids:
            for p in providers:
                if (p.get("id") == tid or p.get("model") == tid) and p not in ordered:
                    ordered.append(p)
        chain_ids = {p.get("id") for p in ordered}
        ...
        # en dessous de la chaine quand le budget calme est epuise.
        if cloud_budget and usage.get("cloud", 0) >= cloud_budget and not _mode_tempete_actif():
            gratuits_ids = _gratuits_actifs()
            chaine = [p for p in providers if p.get("id") in chain_ids]
            filet = [p for p in providers
                     if p.get("id") not in chain_ids and p.get("id") in gratuits_ids]
    ...
    # TRANSPARENCE SUBSTITUTION (GO Christophe 14/09) : on garde qui était DEMANDÉ
    # pour le signaler quand le hub en sert un autre (filet/bascule silencieuse).
    demande_id = only_model if isinstance(only_model, str) and only_model not in ("", "auto") else None
    demande_resolue = None
    if demande_id:
        for p in providers:
            if p.get("id") == demande_id or p.get("model") == demande_id:
                demande_resolue = p
                break
    ...
    for prov in providers:
        ...
        try:
            ...
            # 16/08 (Regle 3) : la chaine (preferee) a droit a la patience ;
            # le FILET (les autres actifs) n'a droit qu'a UN essai rapide (10s).
            est_chain = (not chain_ids) or prov.get("id") in chain_ids
            if est_chain:
                content = call_provider(prov, messages, temperature, max_tokens, timeout_budget=timeout_eff)
            else:
                content = _raw_call(prov, messages, temperature, max_tokens, min(timeout_eff, 10))
                _register_result(prov, True)
            # FAMILLE (16/08, condition 2) : tracer CHAQUE bascule hors-chaine
            # avec un log distinct -> reperer tout de suite une tache qui recourt
            # trop souvent au filet (signe d'une chaine routing.json obsolete).
            if chain_ids and prov.get("id") not in chain_ids:
                log_event("filet-universel", "[FILET UNIVERSEL] « " + prov["name"] + " » a repondu hors chaine", task or "?")
            # TRANSPARENCE SUBSTITUTION : modèle servi ≠ modèle demandé -> event + réponse
            servi_differe = bool(
                demande_resolue is not None
                and (prov.get("id") != demande_resolue.get("id")
                     or prov.get("model") != demande_resolue.get("model"))
            )
            if servi_differe:
                log_event("substitution",
                          "MODELE SUBSTITUE : demandé « " + demande_id + " » → servi « "
                          + prov.get("model", "?") + " » (" + prov.get("name", "?") + ")",
                          task or "?")
            ...
            return {"content": content, "provider": prov["name"],
                    "model": prov.get("model", "?"),
                    "model_demande": demande_id,
                    "substitue": servi_differe,
                    "attempts": attempts}
        except ReseauIndisponible as e:
    ...
    _tache_echec(task)  # 16/08 (Regle 4) : anti-tempete par tache
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/health":
            ...
            self._json(200, {"events": _tail_jsonl(EVENTS_PATH, 25)})
        elif self.path == "/usage":
            self._json(200, {"usage": _tail_jsonl(USAGE_PATH, 50), "today": usage_today()})
        ...
        if self.path == "/v1/chat/completions":
            ...
            try:
                ...
                self._json(200, {
                    ...
                    "model_demande": result.get("model_demande"),
                    "substitue": result.get("substitue", False),
