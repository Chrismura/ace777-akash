...
# Chemins tempête (zone ADA prioritaire) — racine réelle du projet ACE777
STRAT_DIR: str = os.path.expanduser("~/ace777-test-day1/Index_Maison/strategie")
ADA_ALARME: str = os.path.join(STRAT_DIR, "alarme.json")
ADA_GARDIENNE: str = os.path.join(STRAT_DIR, "ada_gardienne_live.json")
ANCIEN_CHEMIN_TEMPETE: str = os.path.join(STRAT_DIR, "etat_tempete.json")
...
def verifier_c2() -> Dict[str, Any]:
    ...
    for p in providers:
        # Gratuit ACTIF : free:true ET enabled n'est pas explicitement false
        # (défaut = actif si le champ est absent)
