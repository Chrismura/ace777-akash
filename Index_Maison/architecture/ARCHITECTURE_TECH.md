...
| **Cortana V2 (LA Cortana)** | `Index_Maison/scripts/cortana_brief.py` · `cortana_cockpit_bridge.py` (`:17777`) · `cortana_thermo.py` | Python · edge-tts (voix Vivienne) · oral_fr (nombres en mots) · barge_in (micro coupe la voix) · yeux (vision à la demande) | VOICE | V2 prod | **LE canal unique vers Cortana** (chat cockpit + voix Vivienne) · brief matin · consultation famille/juge (`/chat`) · `.urgent_alert.json` · **contexte ACE+HULK injecté dans le chat (18/08)** |
| ~~Cortana Rust vocale~~ | `~/crypto-voice-assistant-core/` (Whisper + say Amélie) | Rust séparé | VOICE | ⏸ **VEILLE (18/08)** | **Hors-circuit** : doublon, aucun contexte, voix non utilisable sur le cockpit bureau, LLM en direct sans hub (violerait C9). Gardé (rien ne se supprime) · réactivation = décision + GO |
...
**No single `main.py`.** · **78 LaunchAgents** (audit complet 09/09 : 9 démons KeepAlive + 68 planifiés, 77/77 sources présentes · +1 le 10/09 = `fusibles-mesure` — boucles vérées par fraîcheur réelle, carte complète en `tech.html` §11).
...
|------|--------|---------------|
| **2026-09-10** | **Fusibles par paire branchés dans HULK** (GO Christophe) : budget de perte journalier par actif = `1,5 × σ mesurée` (volatilité réelle des 20 paires — la plus folle bouge 7,8× la plus sage, une règle unique est mathématiquement fausse ; budgets EDEL 3,00 $ → BTC 0,65 $) · **étage 1** : budget consommé → entrées gelées pour la paire seule (sorties toujours libres) · **étage 2** : détarage linéaire 0→100 % du budget (on freine avant le mur) · **dégel SUR DONNÉES, jamais sur horloge** (prix > MM24h ET dd6 > −3 %, capteur launchd `fusibles-mesure` 5 min) · garde-fusible fail-closed côté dégel (mesures > 15 min = périmées → dégel impossible) / fail-open côté capteur (sa mort ne peut jamais provoquer un dégel) · 9/9 tests harnais (gel, détarage, dégel prouvé, reprise multi-CSV, config absente, mesures absentes, gain, inconnu) · double comptage CSV du resume évité (le resume COPIE le journal → scanner le pointeur seul) · 2 redémarrages propres HULK (SIGTERM → save_state → watchdog --resume, état complet repris 13 pos) · registre 49 entrées, veilleuse ✅ STABLE | La protection épouse la nature de chaque actif (fusible par pièce) sans toucher au coupe-circuit général (11,67 %, invariant au régime) · l'étage 2 transforme le gel binaire en frein progressif — la pire vraie journée de HULK (−4,7 %) reste sous le seuil du Mur de Fer |
| **2026-09-09 (3)** | **Disjoncteur C7 rebranché + recalibré** (GO Christophe) : launchd check/60s restauré + capteur `nourrir_disjoncteur.py` créé (le disjoncteur est sourd par design — sans capteur il ne détecte rien ; pivot PnL 00:00 UTC depuis `.hulk_resume_pointer`, fail-safe anti-invention, launchd 60s) · seuils recalibrés **11,67 %/jour** / global **25 %** · **fix dual STOP_ALL** : deux conventions de kill-switch coexistaient (strategie/ vs racine) — le Mur de Fer n'aurait jamais couvert HULK · garde STOP_ALL au watchdog (sinon relance en boucle après Mur de Fer) · 5 tests hors vol OK (OK, Mur de Fer réel, watchdog refus de relance, état déclenché, HULK muet) · POINT_REPRISE 16/08 corrigé (affirmations « branché+inséré » deux fois fausses) | Un kill switch a besoin de trois organes : le décideur (disjoncteur), les oreilles (capteur), le bras (STOP_ALL raccordé aux moteurs) — il en manquait deux sur trois depuis le 16/08 |
...
- Le Risk Guardian lui-même **n'est toujours pas en vol** (kill switch = GO futur). ⚠ **Alerte 09/09 — résolue le jour même** : le **Disjoncteur C7** avait **perdu son plist de LaunchAgents** sans trace dans les journaux (code/config/historique intacts dans le repo). **RÉPARÉ 09/09 (GO Christophe)** : rebranché launchd check/60s + capteur **`nourrir_disjoncteur.py`** créé et en vol (le disjoncteur est sourd par design — le capteur mesure la perte journalière HULK depuis le pivot 00:00 UTC, launchd 60s) + recalibration **11,67 %/jour** / global **25 %** + fix dual STOP_ALL (le Mur de Fer couvre désormais HULK réellement) + garde STOP_ALL au watchdog · 5 tests hors vol OK.
