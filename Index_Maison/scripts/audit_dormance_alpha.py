...
def charger_csvs_fenetre(win_start: datetime, win_end: datetime, specific_files: Optional[List[str]] = None) -> List[Dict[str, Any]]:
...
def generer_rapport_markdown(donnees_A: Dict[str, Any], donnees_B: Dict[str, Any], config_A: Dict[str, str], config_B: Dict[str, str]) -> str:
    ...
    md.append(f"**Date de génération** : {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}  ")
    ...
    md.append(f"| **Cadence (cycles/min)** | α={a_alpha['cadence_cpm']:.1f} β={a_beta['cadence_cpm']:.1f} | α={b_alpha['cadence_cpm']:.1f} β={b_beta['cadence_cpm']:.1f} |")
    md.append(f"| **Silences ≥ 5 min** | α={a_alpha['silences_gt_5m']} β={a_beta['silences_gt_5m']} | α={b_alpha['silences_gt_5m']} β={b_beta['silences_gt_5m']} |")
    ...
    # Conclusion générale — chiffres DÉRIVÉS des données, jamais en dur
    ta = b_alpha['taux_remplissage']
    ta_ref = a_alpha['taux_remplissage']
    tb = b_beta['taux_remplissage']
    pnl_a_b = b_alpha['pnl_total']
    md.append("## 5. Verdict Synthétique pour Christophe\n")
    md.append(f"- **Asymétrie confirmée** : ALPHA encaisse une baisse de régime sévère en Fenêtre B ({ta:.1f}% de fills) par rapport à sa forme de la Fenêtre A ({ta_ref:.1f}%), pendant que BETA reste stable ({tb:.1f}%).")
    md.append(f"- **PnL ALPHA** : {a_alpha['pnl_total']:+.2f} → {pnl_a_b:+.2f} (le creux de régime s'accompagne d'une perte).")
    md.append(f"- **Portes verrouillées** : radar_block domine dans les deux fenêtres ({a_alpha['skips_par_raison'].get('radar_block', 0)} → {b_alpha['skips_par_raison'].get('radar_block', 0)} skips ALPHA). Si la config est inchangée (non vérifiable ici), c'est le comportement du flux de prix par rapport aux seuils stricts d'ALPHA qui coince.")
    md.append("- **Prochaine étape** : vérifier la config réelle dans les logs T1_console des deux jours, puis relâcher éventuellement `RADAR_MIN_CONF_ALPHA` ou inspecter la volatilité spécifique du sous-jacent d'ALPHA.")
...
def main() -> None:
    ...
    lignes_A_alpha = charger_csvs_fenetre(WIN_A_START, WIN_A_END, [f for f in csvs_A if "ALPHA" in f])
    lignes_A_beta  = charger_csvs_fenetre(WIN_A_START, WIN_A_END, [f for f in csvs_A if "BETA" in f])
    
    # 2. Collecte Fenêtre B (14/07 journée)
    # NB: ne garder QUE les fichiers écrits PENDANT le 14/07 (mtime dans la
    # fenêtre) OU dont le nom porte explicitement la date 20260714. Un simple
    # ">= start" inclurait les fichiers cumulatifs écrits jusqu'au 12/08
    # (ex. NUAGE_PROD_4H_ALPHA.csv, 59k lignes) et polluerait le PnL.
    print("[*] Chargement des données Fenêtre B (14/07)...")
    tous_csvs = glob.glob(os.path.join(RUNS_DIR, "*.csv"))
    csvs_B = [
        f for f in tous_csvs
        if (WIN_B_START.timestamp() <= os.path.getmtime(f) <= WIN_B_END.timestamp())
        or "20260714" in f
    ]
    
    lignes_B_alpha = charger_csvs_fenetre(WIN_B_START, WIN_B_END, [f for f in csvs_B if "ALPHA" in f])
    lignes_B_beta  = charger_csvs_fenetre(WIN_B_START, WIN_B_END, [f for f in csvs_B if "BETA" in f])
