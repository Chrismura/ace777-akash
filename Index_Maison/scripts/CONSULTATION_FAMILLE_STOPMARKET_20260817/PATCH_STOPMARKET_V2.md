# PATCH STOP_MARKET V2 — Filet de sécurité physique Binance

**Date :** 17/08/2026 — **V2** (intègre les 3 réserves du codeur, AVIS_CODEUR_V2.md)
**Cible :** `genesis_manifest.txt` (setup A, md5 `fe2a7bcc9dc1f31bd524ffc433f9186d` = `genesis_manifest.txt.BAK_avant_fix_heartbeat_20260815-152847`)
**Validations :** round table famille 17/08 (5/5) · codeur V1 (GO-AVEC-RÉSERVE 85%) · codeur V2 (GO-AVEC-RÉSERVE 90%)
**Règle :** rien n'est intégré tant que la famille (2 + juge) n'a pas validé cette V2 concrète.
...
## Δ V1 → V2 (réserves codeur intégrées)

1. **`&workingType=MARK_PRICE`** ajouté dans `$q_stop` (réserve n°1 — filet déclenché sur le mark price, pas sur le dernier prix manipulable).
2. **Cancel ACESTOP ajouté à la fin de `duo_v63_phase_shift_close()`** (réserve n°2 — le seul chemin de sortie hors bloc commun).
3. **Arrondi directionnel sécurisé** (réserve n°3 — `floor` pour un stop LONG, `ceil` pour un stop SHORT, jamais vers le centre du marché).
...
Point d'ancrage réel : juste après `entry_ts_iso="$(date -u +%FT%TZ)"`, avant `reason="timeout"`. On a `$entry_price`, `$qty`, `$side`, `$close_side`, `$trade_position_side_param` à disposition. **V2 : `workingType=MARK_PRICE` + arrondi directionnel** :

```bash
  entry_ts_iso="$(date -u +%FT%TZ)"

  # ============================================================
  # FILET PHYSIQUE : STOP_MARKET posé chez Binance (patch 17/08, V2)
  ...
  if [ "$ACE_STOP_MARKET_ENABLED" = "TRUE" ]; then
    ...
    if [ "$side" = "BUY" ]; then
      ...
      # Arrondi FLOOR (plancher) pour un LONG : le stop ne monte JAMAIS vers le marché
      ...
      # Arrondi CEIL (plafond) pour un SHORT : le stop ne descend JAMAIS vers le marché
    ...
    ts="$(now_ms)"
    q_stop="symbol=$SYMBOL&side=$close_side&type=STOP_MARKET&quantity=$qty&stopPrice=$stop_price&workingType=MARK_PRICE&reduceOnly=true&positionSide=$stop_position_side&newClientOrderId=ACESTOP${i}&timestamp=$ts&recvWindow=$RECV_WINDOW"
...
⚠️ **Sécurité fenêtre d'entrée** : entre l'entrée MARKET et le placement du stop (~200-500 ms), la position est sans filet. Si le placement échoue → on log `FILET_ERROR` et on **continue quand même** (le stop logiciel reste actif, la boucle de gestion tourne) — jamais de position ouverte sans boucle de gestion.

⚠️ **Piège idempotence (codeur V2)** : si un cycle plante et laisse `ACESTOP${i}` actif, le cycle suivant utilisera le MÊME `newClientOrderId` → Binance rejette → trade SANS filet. C'est pourquoi le heartbeat (morceau 5) doit raser les orphelins **avant** l'entrée — ordre strict : heartbeat → entrée → stop.

### 4. Cancel dans le bloc de sortie COMMUN (après `exit_resp`, ligne ~2437)

Le point structurel : TOUS les chemins de sortie (stop_loss, trailing, shockwave, fluid, timeout, target) passent par `exit_resp="$(private_post "/fapi/v1/order" "$q_exit" || true)"`. On injecte le cancel juste après la réussite de la sortie MARKET :
...
### 4bis. Cancel dans le chemin PHASE_SHIFT (V2 — réserve codeur n°2)

`duo_v63_phase_shift_close()` (ligne ~1307) ferme la position en 3 étapes (13/8/5) et ne passe PAS par le bloc commun. À la fin de la fonction, la position est à 0 → le filet doit être détruit là aussi. À insérer juste avant le `[ "$ok_any" -eq 1 ]` final :

```bash
  # SÉCURITÉ PHASE SHIFT (patch 17/08 V2) : position fermée, on nettoie le filet
  if [ "$ACE_STOP_MARKET_ENABLED" = "TRUE" ]; then
    private_delete_order_sniper "$SYMBOL" "$i"
  fi
  [ "$ok_any" -eq 1 ]
...
⚠️ Attention : dans cette fonction, la variable `$i` (numéro de cycle) n'est pas définie localement — elle reste accessible en global (le moteur est un script unique). À vérifier lors de l'intégration que `$i` est bien le cycle courant à cet endroit (sinon passer le cycle_id en argument).

### 5. Heartbeat anti-orphelin (début de CHAQUE cycle, ligne ~1508)

Au début de la boucle `for i in $(seq 1 "$CYCLES"); do`, avant toute entrée — on rase les ACESTOP restants d'un cycle planté. **Ordre strict : heartbeat → entrée → stop** (piège idempotence V2) :
...
for i in $(seq 1 "$CYCLES"); do
  ...
  # (aucun vieux stop ne peut parasiter le trade suivant — idempotence)
  # ============================================================
  if [ "$ACE_STOP_MARKET_ENABLED" = "TRUE" ]; then
    ts="$(now_ms)"
...
⚠️ `allOpenOrders` rase TOUT sur la paire. **Aujourd'hui** seuls les ACESTOP dorment (entrées/sorties = MARKET immédiats) → sûr. **Réserve codeur V2** : si un jour on ajoute des ordres limites qui dorment (take-profit partiel, grille), il faudra cibler le nettoyage par `/fapi/v1/openOrders` filtré sur clientOrderId au lieu du rasage aveugle. OK pour V1/testnet.
...
## Points de vigilance (codeur V2 + binôme)

1. **recvWindow / horloge NTP** : les DELETE+POST rapprochés exigent une horloge système propre, sinon erreur `-1021`. Le moteur utilise déjà `now_ms()` partout — pas de changement de mécanique, mais à surveiller en test.
2. **Idempotence** : le heartbeat DOIT tourner avant l'entrée (ordre strict) pour ne jamais réutiliser un `ACESTOP${i}` encore vivant.
3. **Coexistence stop logiciel (7 bps soft / 16 bps hard) + filet (8-10 bps)** : le filet ne se déclenche que si la boucle meurt ou traîne. Aucun conflit attendu.
4. **Fenêtre d'entrée sans filet** (~200-500 ms) : acceptable en testnet ; si le placement échoue, on log `FILET_ERROR` et on continue (jamais de position sans gestion).
