#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v2_confirmation_replay_var_veto_relatif_v2.py — VARIANTE v2 (DIRECTIVE DE SUPERVISION
Christophe 17/09, exécution à la lettre). Zéro ordre, zéro euro.

SPÉCIFICATION FIGÉE AVANT LE RUN : identique à la spec scellée du 12/09 SAUF le veto :
  seuil_veto = (avg30 * 1.2) if (avg30 is not None and avg30 > 0) else 0.0001
  veto actif si dernier_f < seuil_veto.
...
OUT_JSON = BASE / "Index_Maison/thermo/v2_confirmation_resultat_veto_relatif_v2.json"
...
def run():
    ...
    for i in range(31, len(jours)):
        ...
        if pos is None and i > 31:
            ...
            # veto zone morte — DIRECTIVE DE SUPERVISION (formule exacte du propriétaire)
            seuil_veto = (avg30 * 1.2) if (avg30 is not None and avg30 > 0) else 0.0001
            if dernier_f is not None and dernier_f < seuil_veto:
                continue
    ...
    res = {
        ...
        "spec": "VARIANTE v2 directive supervision 17/09 : veto seuil_veto=(avg30*1.2 si avg30>0 sinon 0.0001) — formule exacte du propriétaire ; reste = spec scellée 12/09",
