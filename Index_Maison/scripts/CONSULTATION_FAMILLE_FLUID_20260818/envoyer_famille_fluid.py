#!/usr/bin/env python3
"""Validation famille — fluid_exit_inversion (18/08/2026)
...
PROMPT = open(os.path.join(D, "QUESTION_FAMILLE_FLUID.md"), encoding="utf-8").read()

MEMBRES = ["gemini", "groq"]  # 2 membres (les plus réactifs, vérifiés vivants)
JUGE = "signets.juge"         # le juge (nara) — tranche toujours

SYSTEM = (
    "Tu es un membre du conseil de la famille ACE777, consulté pour une décision de configuration. "
...
SYSTEM_JUGE = (
    ...
    "Ton rôle : trancher. Tu lis les preuves et tu rends la décision finale, en connaissance de cause. "
...
if __name__ == "__main__":
    for m in MEMBRES:
        send(m, m, SYSTEM,
             os.path.join(D, "AVIS_FAMILLE_FLUID_%s.md" % m),
             os.path.join(D, "AVIS_FAMILLE_FLUID_%s.json" % m))
    send("JUGE", JUGE, SYSTEM_JUGE,
         os.path.join(D, "AVIS_FAMILLE_FLUID_JUGE.md"),
         os.path.join(D, "AVIS_FAMILLE_FLUID_JUGE.json"))
