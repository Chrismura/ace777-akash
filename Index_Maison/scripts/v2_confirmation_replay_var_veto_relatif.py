#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v2_confirmation_replay_var_veto_relatif.py — VARIANTE UNIQUE (GO Christophe 17/09)
de v2_confirmation_replay.py (scellé 12/09). Zéro ordre, zéro euro.

SPÉCIFICATION FIGÉE AVANT LE RUN : identique à la spec scellée du 12/09 (dossier §3)
SAUF UNE SEULE VARIABLE — VETO ZONE MORTE : seuil statique 0.0002 remplacé par
seuil RELATIF funding < avg30 × 1.2. Tout le reste est intact (portes, sorties,
masques, critères, conventions économiques).
...
OUT_JSON = BASE / "Index_Maison/thermo/v2_confirmation_resultat_veto_relatif.json"
...
def run():
    ...
    for i in range(31, len(jours)):
        ...
        if pos is None and i > 31:
            ...
            # veto zone morte — VARIANTE GO 17/09 : seuil RELATIF (avg30 × 1.2)
            if dernier_f is not None and avg30 is not None and dernier_f < avg30 * 1.2:
                continue
    ...
    res = {
        ...
        "spec": "VARIANTE GO 17/09 : spec scellée 12/09 (§3), SEULE variable = veto relatif avg30×1.2 (remplace 0.0002 statique)",
