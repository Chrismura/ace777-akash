#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Envoyer la consultation SECTEURS à la famille (2 membres + juge).
...
QUESTION = os.path.join(D, "QUESTION_FAMILLE_SECTEURS.md")
...
SYSTEM_MEMBRE = (
    "Tu es un membre du conseil de la famille ACE777. Christophe a demandé un diagnostic "
    "du système par secteurs pour trouver le goulot d'étranglement n°1, avant d'installer "
    "les pépites des signets. Réponds en français, 3 phrases max par question, tranche net : "
    "GO / GO-AVEC-RÉSERVE / NON. Si tu proposes une amélioration, dis-la en 1 phrase."
)
...
SYSTEM_JUGE = (
    ...
    "avec 3 phrases max par point et les conditions non négociables si GO-AVEC-RÉSERVE."
)
...
def main():
    ...
    for m in MEMBRES:
        t = threading.Thread(target=appeler, args=(
            "famille.%s.secteurs" % m, SYSTEM_MEMBRE, question,
            os.path.join(D, "AVIS_FAMILLE_SECTEURS_%s.md" % m),
            os.path.join(D, "AVIS_FAMILLE_SECTEURS_%s.json" % m),
        ))
        t.start()
        threads.append(t)
    for t in threads:
    ...
    avis = []
    for m in MEMBRES:
        p = os.path.join(D, "AVIS_FAMILLE_SECTEURS_%s.md" % m)
        if os.path.exists(p):
            avis.append("## Avis %s\n%s" % (m, open(p, encoding="utf-8").read()))
    question_juge = question + "\n\n---\n\nAVIS DES MEMBRES :\n\n" + "\n\n".join(avis)
    appeler(JUGE, SYSTEM_JUGE, question_juge,
            os.path.join(D, "AVIS_FAMILLE_SECTEURS_JUGE.md"),
            os.path.join(D, "AVIS_FAMILLE_SECTEURS_JUGE.json"))
