...
# -*- coding: utf-8 -*-
"""
Dialogue GEMINI R32 — RAPPORT J+1 SHADOW SCÉNARIO C (03/09).
Même mécanisme que dialogue_gemini_poussiere.py : un tour, réponse archivée.
Usage :
  python3 dialogue_gemini_r32.py            # envoie GEMINI_MSG_R32.txt, archive la réponse
"""
import json
import os
import time
...
MSG_FILE = HERE / "GEMINI_MSG_R32.txt"
OUT_FILE = HERE / "GEMINI_R32_REPONSE_J1.md"
...
def main():
    ...
    print(f"ENVOI R32 → {MODEL} ({len(msg)} caractères)…", flush=True)
    try:
        reponse, dt = ask(msg)
        ...
        raise SystemExit(1)
    OUT_FILE.write_text(
        f"# RÉPONSE GEMINI R32 — RAPPORT J+1 SHADOW (reçue en {dt:.0f}s)\n\n{reponse}\n",
        encoding="utf-8")
