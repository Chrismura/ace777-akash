# Réponse codeur (provider Google Gemini, 9.7s)
...
# URL du hub local (standard de l'écosystème)
HUB_URL = "http://127.0.0.1:8787/chat"
...
def load_recent_cortana_analyses(limit=3):
                    ...
                    for line in f:
                        ...
                        if line:
                            try:
                                ...
                                # S'assurer qu'il y a un timestamp ou trier par ts
    ...
    # Tri par timestamp si présent (ts ou timestamp)
    def get_ts(item):
        return item.get("ts", item.get("timestamp", 0))
    
    all_lines.sort(key=get_ts, reverse=True)
    ...
    if os.path.exists(JUSTESSE_PATH):
        try:
            with open(JUSTESSE_PATH, "r", encoding="utf-8") as f:
                data = json.load(f)
                return data.get("pct", "Inconnu")
        except Exception:
            pass
    return "Inconnu"

def call_hub(prompt, task_name="quant_desk", retries=2, delay=4):
    """Appelle le hub local avec robustesse (retry + fail-open)."""
    payload = {
        "task": task_name,
        "prompt": prompt
    }
        ...
        try:
            with urllib.request.urlopen(req, timeout=None) as response:
                if response.status == 200:
                    ...
                    # Extraction standard de la réponse du hub selon les conventions
                    return res_data.get("response", res_data.get("text", str(res_data)))
        except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, socket_timeout if 'socket_timeout' in globals() else Exception):
            if attempt < retries:
                time.sleep(delay)
            else:
                pass
