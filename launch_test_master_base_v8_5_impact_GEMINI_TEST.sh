...
# === MASTER BASE V8.5 IMPACT — GEMINI TEST (NE PAS REMPLACER L'ORIGINAL) ===
# Original intact : launch_test_master_base_v8_5_impact.sh
# Backup          : launch_test_master_base_v8_5_impact.ORIGINAL.sh
#
# Test :
#   LAUNCH_V85_SCRIPT=./launch_test_master_base_v8_5_impact_GEMINI_TEST.sh \
#     ./launch_vortex_v2_collab_4h_binance.sh
#
# Corrections vs patch Gemini brut :
# - SIGPIPE : trap PIPE + pipefail off (logs live conservés — pas de fichier tmp bloquant 4h)
# - Levier  : x13 fixe (patch Gemini) OU rampe 5→13 si ALPHA_RAMP_MODE=model (+25 USDT)
...
echo "=== GEMINI_TEST launcher actif (original non modifié) ==="
...
launch_alpha() {
  (
    ...
    if [ "${ALPHA_RAMP_MODE:-gemini}" = "model" ]; then
      export LEVERAGE_RAMP_START="5"
      export LEVERAGE_RAMP_END="13"
      echo "[ALPHA_X13_BURST13] GEMINI_TEST ramp=model (5->13 comme run +25 USDT)"
    else
      export LEVERAGE_RAMP_START="13"
      export LEVERAGE_RAMP_END="13"
      echo "[ALPHA_X13_BURST13] GEMINI_TEST ramp=gemini (x13 fixe dès cycle 1)"
    fi
...
if [ -f "$beta_csv" ] && [ -f "$alpha_csv" ]; then
  ...
  echo
  echo "=== RAPPORT PNL 3 PARTIES (GEMINI_TEST) ==="
