# RAPPORT PNL AUTO — NUAGE_PROD_4H

**Session:** `NUAGE_PROD_4H`
**Période:** 2026-07-22T11:45:04Z → 2026-07-22T14:22:52Z (2h37m)
**Setup:** `vide_froid_binance` v`V2.2.1_NO_SUICIDE` | BETA `200` USDT | ALPHA `800` USDT | LLM gate `TRUE` fail_closed=`TRUE`
**Généré:** 2026-07-22T14:32:44Z UTC
**Filtre session:** `ts >= 2026-07-22T11:44:55Z` (lignes CSV antérieures exclues)

---

## BILAN GLOBAL

| Métrique | Valeur |
|----------|--------|
| **PNL BETA** | **-0.7306 USDT** |
| **PNL ALPHA** | **+0.1696 USDT** |
| **PNL SESSION TOTAL** | **-0.5609 USDT** |
| Statut | `NEGATIF` |

---

## BETA — BETA (SCOUT x5)

| Métrique | Valeur |
|----------|--------|
| Trades FILLED | 51 |
| Gagnants | 20 |
| Perdants | 23 |
| Flat (0) | 8 |
| Win rate | **39.2%** |
| Gains totaux | +2.0024 USDT |
| Pertes totales | -2.7329 USDT |
| **PNL net** | **-0.7306 USDT** |
| BPS moyen | -0.46 |

**Meilleur trade:** +0.4298 USDT
**Pire trade:** -0.6713 USDT

**Direction:** SELL (26), BUY (25)

**Raisons de sortie (exitReason):**
| Raison | Nb |
|--------|-----|
| shock_inversion_stop | 36 |
| fluid_exit_inversion | 9 |
| trailing_stop | 3 |
| fluid_exit_brake | 1 |
| shock_exit_10bps | 1 |

**Cycles SKIP:** 862
| Raison | Nb |
|--------|-----|
| radar_block | 688 |
| impulse_resonance_wait | 98 |
| vacuum_filter | 71 |
| tactic_mismatch | 5 |

---

## ALPHA — ALPHA (HUNTER x13)

| Métrique | Valeur |
|----------|--------|
| Trades FILLED | 5 |
| Gagnants | 1 |
| Perdants | 1 |
| Flat (0) | 3 |
| Win rate | **20.0%** |
| Gains totaux | +1.2118 USDT |
| Pertes totales | -1.0421 USDT |
| **PNL net** | **+0.1696 USDT** |
| BPS moyen | -0.05 |

**Meilleur trade:** +1.2118 USDT
**Pire trade:** -1.0421 USDT

**Direction:** SELL (4), BUY (1)

**Raisons de sortie (exitReason):**
| Raison | Nb |
|--------|-----|
| shock_inversion_stop | 4 |
| trailing_stop | 1 |

**Cycles SKIP:** 1008
| Raison | Nb |
|--------|-----|
| radar_block | 750 |
| impulse_resonance_wait | 109 |
| vacuum_filter | 77 |
| tension_stale | 67 |
| tactic_mismatch | 5 |

---

## SYNTHÈSE

| Indicateur | BETA | ALPHA | TOTAL |
|------------|------|-------|-------|
| Trades | 51 | 5 | 56 |
| PnL | -0.7306 | +0.1696 | **-0.5609** |
| Win rate | 39.2% | 20.0% | 37.5% |

## IRM — régimes de tension (proxy, lecture seule)

> Pas un modèle ARCH Engle. Classification sur `tension=` des cycles BETA. N'influence pas le moteur. Seuils: COMPRESSÉ `< 0.05` · CLUSTER `≥ 1.0` · sinon TRANSITOIRE.

| Régime | Cycles | % temps | Fills | PnL fills (USDT) |
|--------|--------|---------|-------|------------------|
| COMPRESSÉ (attente à froid) | 620 | 67.9% | 0 | +0.0000 |
| TRANSITOIRE (bruit retail) | 157 | 17.2% | 0 | +0.0000 |
| CLUSTER (tension haute — proxy) | 136 | 14.9% | 51 | -0.7306 |

- Fenêtre: `2026-07-22T11:45:04Z` → `2026-07-22T14:22:52Z` (913 cycles) · μ(tension)=0.5949 · σ=1.7163 · courant(proxy)=**COMPRESSÉ (attente à froid)**
- Source: `NUAGE_PROD_4H_BETA_X5.csv`

## Engle — couches évolutives (hors moteur)

- Plan: `engle/PLAN_COUCHES_B1_B3.md`
- Journal B1: `engle/journal/ENGLE_JOURNAL_DERNIER.md` (généré via `engle_journal.rb` / `update_state_md.sh`)
- Adapt B2: `ENGLE_ADAPT=0` (défaut OFF = usine pure ; `log` = posture JSON only)
- Dernière posture log: `WAIT_COLD` · régime `COMPRESSE` · applied=`false`

## CONFIG ACTIVE (snapshot)

- ENTRY_25_75 BETA: `0.70` | ALPHA: `0.50`
- SHOCK_EXIT: `16` bps
- VOLATILITY_FILTER: `16`
- STASE: spread=`16` vol=`16`
- POLL_SEC: `0.064`

---

*Rapport auto — CSV: `NUAGE_PROD_4H_BETA_X5.csv` | `NUAGE_PROD_4H_ALPHA_X13_BURST13.csv`*
*STATE: `runs/STATE.md`*
