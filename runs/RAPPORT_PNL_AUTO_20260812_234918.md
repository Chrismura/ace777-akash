# RAPPORT PNL AUTO — MASTER_VORTEX_V2_COLLAB_4H

**Session:** `MASTER_VORTEX_V2_COLLAB_4H`
**Période:** 2026-08-12T23:28:00Z → 2026-08-12T23:49:08Z (0h21m)
**Setup:** `vide_froid_vortex_v2_collab` v`2026-07-10-v2.2.2-no-partner-halt` | BETA `200` USDT | ALPHA `800` USDT | LLM gate `TRUE` fail_closed=`TRUE`
**Généré:** 2026-08-12T23:49:18Z UTC
**Filtre session:** `ts >= 2026-08-12T23:27:36Z` (lignes CSV antérieures exclues)

---

## BILAN GLOBAL

| Métrique | Valeur |
|----------|--------|
| **PNL BETA** | **-0.1175 USDT** |
| **PNL ALPHA** | **+0.3771 USDT** |
| **PNL SESSION TOTAL** | **+0.2596 USDT** |
| Statut | `POSITIF` |

---

## BETA — BETA (SCOUT x5)

| Métrique | Valeur |
|----------|--------|
| Trades FILLED | 7 |
| Gagnants | 1 |
| Perdants | 6 |
| Flat (0) | 0 |
| Win rate | **14.3%** |
| Gains totaux | +0.0164 USDT |
| Pertes totales | -0.1339 USDT |
| **PNL net** | **-0.1175 USDT** |
| BPS moyen | -0.45 |

**Meilleur trade:** +0.0164 USDT
**Pire trade:** -0.0465 USDT

**Direction:** SELL (7)

**Raisons de sortie (exitReason):**
| Raison | Nb |
|--------|-----|
| shock_inversion_stop | 4 |
| fluid_exit_inversion | 2 |
| fluid_exit_brake | 1 |

**Cycles SKIP:** 72
| Raison | Nb |
|--------|-----|
| radar_block | 35 |
| gap_guard_pause | 27 |
| impulse_resonance_wait | 7 |
| stase_ecoute | 2 |
| tactic_mismatch | 1 |

---

## ALPHA — ALPHA (HUNTER x13)

| Métrique | Valeur |
|----------|--------|
| Trades FILLED | 5 |
| Gagnants | 3 |
| Perdants | 1 |
| Flat (0) | 1 |
| Win rate | **60.0%** |
| Gains totaux | +0.5246 USDT |
| Pertes totales | -0.1475 USDT |
| **PNL net** | **+0.3771 USDT** |
| BPS moyen | 0.10 |

**Meilleur trade:** +0.1804 USDT
**Pire trade:** -0.1475 USDT

**Direction:** BUY (5)

**Raisons de sortie (exitReason):**
| Raison | Nb |
|--------|-----|
| shock_inversion_stop | 4 |
| fluid_exit_brake | 1 |

**Cycles SKIP:** 89
| Raison | Nb |
|--------|-----|
| radar_block | 77 |
| impulse_resonance_wait | 11 |
| duo_wait | 1 |

---

## SYNTHÈSE

| Indicateur | BETA | ALPHA | TOTAL |
|------------|------|-------|-------|
| Trades | 7 | 5 | 12 |
| PnL | -0.1175 | +0.3771 | **+0.2596** |
| Win rate | 14.3% | 60.0% | 33.3% |

## IRM — régimes de tension (proxy, lecture seule)

> Pas un modèle ARCH Engle. Classification sur `tension=` des cycles BETA. N'influence pas le moteur. Seuils: COMPRESSÉ `< 0.05` · CLUSTER `≥ 1.0` · sinon TRANSITOIRE.

| Régime | Cycles | % temps | Fills | PnL fills (USDT) |
|--------|--------|---------|-------|------------------|
| COMPRESSÉ (attente à froid) | 63 | 79.7% | 0 | +0.0000 |
| TRANSITOIRE (bruit retail) | 9 | 11.4% | 0 | +0.0000 |
| CLUSTER (tension haute — proxy) | 7 | 8.9% | 7 | -0.1175 |

- Fenêtre: `2026-08-12T23:28:00Z` → `2026-08-12T23:43:47Z` (79 cycles) · μ(tension)=0.2893 · σ=0.8834 · courant(proxy)=**COMPRESSÉ (attente à froid)**
- Source: `MASTER_VORTEX_V2_COLLAB_4H_BETA_X5.csv`

## Engle — couches évolutives (hors moteur)

- Plan: `engle/PLAN_COUCHES_B1_B3.md`
- Journal B1: `engle/journal/ENGLE_JOURNAL_DERNIER.md` (généré via `engle_journal.rb` / `update_state_md.sh`)
- Adapt B2: `ENGLE_ADAPT=0` (défaut OFF = usine pure ; `log` = posture JSON only)
- Dernière posture log: `WAIT_COLD` · régime `COMPRESSE` · applied=`false`

## CONFIG ACTIVE (snapshot)

- ENTRY_25_75 BETA: `0.70` | ALPHA: `0.50`
- SHOCK_EXIT: `16` bps
- VOLATILITY_FILTER: `16`
- STASE: spread=`16` vol=`16`
- POLL_SEC: `0.064`

---

*Rapport auto — CSV: `MASTER_VORTEX_V2_COLLAB_4H_BETA_X5.csv` | `MASTER_VORTEX_V2_COLLAB_4H_ALPHA_X13_BURST13.csv`*
*STATE: `runs/STATE.md`*
