# RAPPORT PNL AUTO — MASTER_VORTEX_V2_COLLAB_4H

**Session:** `MASTER_VORTEX_V2_COLLAB_4H`
**Période:** 2026-08-13T00:18:12Z → 2026-08-13T00:42:44Z (0h24m)
**Setup:** `vide_froid_vortex_v2_collab` v`2026-07-10-v2.2.2-no-partner-halt` | BETA `200` USDT | ALPHA `800` USDT | LLM gate `TRUE` fail_closed=`TRUE`
**Généré:** 2026-08-13T00:42:53Z UTC
**Filtre session:** `ts >= 2026-08-13T00:17:59Z` (lignes CSV antérieures exclues)

---

## BILAN GLOBAL

| Métrique | Valeur |
|----------|--------|
| **PNL BETA** | **-0.4116 USDT** |
| **PNL ALPHA** | **+3.0258 USDT** |
| **PNL SESSION TOTAL** | **+2.6142 USDT** |
| Statut | `POSITIF` |

---

## BETA — BETA (SCOUT x5)

| Métrique | Valeur |
|----------|--------|
| Trades FILLED | 5 |
| Gagnants | 1 |
| Perdants | 3 |
| Flat (0) | 1 |
| Win rate | **20.0%** |
| Gains totaux | +0.0009 USDT |
| Pertes totales | -0.4126 USDT |
| **PNL net** | **-0.4116 USDT** |
| BPS moyen | -1.82 |

**Meilleur trade:** +0.0009 USDT
**Pire trade:** -0.2828 USDT

**Direction:** SELL (5)

**Raisons de sortie (exitReason):**
| Raison | Nb |
|--------|-----|
| shock_inversion_stop | 5 |

**Cycles SKIP:** 105
| Raison | Nb |
|--------|-----|
| radar_block | 99 |
| impulse_resonance_wait | 3 |
| tactic_mismatch | 2 |
| stase_ecoute | 1 |

---

## ALPHA — ALPHA (HUNTER x13)

| Métrique | Valeur |
|----------|--------|
| Trades FILLED | 7 |
| Gagnants | 3 |
| Perdants | 3 |
| Flat (0) | 1 |
| Win rate | **42.9%** |
| Gains totaux | +3.7732 USDT |
| Pertes totales | -0.7474 USDT |
| **PNL net** | **+3.0258 USDT** |
| BPS moyen | 0.33 |

**Meilleur trade:** +2.2413 USDT
**Pire trade:** -0.5394 USDT

**Direction:** BUY (7)

**Raisons de sortie (exitReason):**
| Raison | Nb |
|--------|-----|
| shock_inversion_stop | 6 |
| fluid_exit_brake | 1 |

**Cycles SKIP:** 108
| Raison | Nb |
|--------|-----|
| radar_block | 94 |
| impulse_resonance_wait | 10 |
| duo_wait | 2 |
| stase_ecoute | 1 |
| tactic_mismatch | 1 |

---

## SYNTHÈSE

| Indicateur | BETA | ALPHA | TOTAL |
|------------|------|-------|-------|
| Trades | 5 | 7 | 12 |
| PnL | -0.4116 | +3.0258 | **+2.6142** |
| Win rate | 20.0% | 42.9% | 33.3% |

## IRM — régimes de tension (proxy, lecture seule)

> Pas un modèle ARCH Engle. Classification sur `tension=` des cycles BETA. N'influence pas le moteur. Seuils: COMPRESSÉ `< 0.05` · CLUSTER `≥ 1.0` · sinon TRANSITOIRE.

| Régime | Cycles | % temps | Fills | PnL fills (USDT) |
|--------|--------|---------|-------|------------------|
| COMPRESSÉ (attente à froid) | 96 | 87.3% | 0 | +0.0000 |
| TRANSITOIRE (bruit retail) | 8 | 7.3% | 0 | +0.0000 |
| CLUSTER (tension haute — proxy) | 6 | 5.5% | 5 | -0.4116 |

- Fenêtre: `2026-08-13T00:18:12Z` → `2026-08-13T00:40:39Z` (110 cycles) · μ(tension)=0.1956 · σ=1.1429 · courant(proxy)=**COMPRESSÉ (attente à froid)**
- Source: `MASTER_VORTEX_V2_COLLAB_4H_BETA_X5.csv`

## Engle — couches évolutives (hors moteur)

- Plan: `engle/PLAN_COUCHES_B1_B3.md`
- Journal B1: `engle/journal/ENGLE_JOURNAL_DERNIER.md` (généré via `engle_journal.rb` / `update_state_md.sh`)
- Adapt B2: `ENGLE_ADAPT=0` (défaut OFF = usine pure ; `log` = posture JSON only)
- Dernière posture log: `WAIT_COLD` · régime `COMPRESSE` · applied=`false`

## CONFIG ACTIVE (snapshot)

- ENTRY_25_75 BETA: `0.70` | ALPHA: `0.50`
- SHOCK_EXIT: `16` bps
- VOLATILITY_FILTER: `16`
- STASE: spread=`16` vol=`16`
- POLL_SEC: `0.064`

---

*Rapport auto — CSV: `MASTER_VORTEX_V2_COLLAB_4H_BETA_X5.csv` | `MASTER_VORTEX_V2_COLLAB_4H_ALPHA_X13_BURST13.csv`*
*STATE: `runs/STATE.md`*
