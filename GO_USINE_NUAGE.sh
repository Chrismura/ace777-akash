...
EXPECT_MD5_PREFIX="64fb153f"   # 21/08 re-scellé patch filet (-4116 clientAlgoId unique + -2021 retry distance) sur 01c38510
DURATION="${1:-04:00:00}"
TAG="${2:-NUAGE_PROD_4H}"
...
if [ "${NUAGE_STORM_LATCH}" = "1" ] || [ "${NUAGE_STORM_LATCH}" = "TRUE" ]; then
  ...
  [[ "$_md5_disk" == 64fb153f* ]] || fail "champion disque altéré"
