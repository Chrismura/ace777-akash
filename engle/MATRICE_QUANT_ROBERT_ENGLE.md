...
4. **B1 Journal + B2 Adapt log (2026-07-19) :** voir `engle/PLAN_COUCHES_B1_B3.md`  
   - `scripts/engle_journal.rb` → `engle/journal/` (post-run / manuel)  
   - `scripts/engle_adapt.rb` + `ENGLE_ADAPT=0` défaut ; `=log` → posture JSON **sans** knobs  
   - Boot `GO_USINE` affiche la ligne B2 ; **aucune** modification de seuils live

5. **Garde-fou duo PID (2026-07-19) :** `engle/DUO_PID_WATCHDOG.md`  
   - Patch runtime `GO_USINE` : relance **BETA et ALPHA** si process mort (`kill -0` sur wrappers)  
   - `NUAGE_DUO_PID_WATCHDOG=1` défaut ; `=0` = OFF (usine pure)  
   - Complète le watchdog sémantique (ALPHA heartbeat only)

6. **Interdits agent :** ne pas modifier `genesis_manifest.txt` / champion ; commande de run **dans le terminal de Christophe** ; topo avant action coûteuse.
...
**Prochaine étape (GO séparé) :** B3 = un seul knobs env A/B 4h, seulement si journaux montrent CLUSTER réel — pas en marché COMPRESSÉ.
