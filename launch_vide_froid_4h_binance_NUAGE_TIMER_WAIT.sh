...
export ACE777_GENESIS_SOURCE="${ACE777_GENESIS_SOURCE:-${ACE777_ROOT}/genesis_manifest.txt}"
...
echo "SWARM=ON | BETA x${BETA_LEVERAGE_OVERRIDE:-5} | GATE=${NUAGE_TENSION_MAX_AGE_MS:-800}ms | STORM_HOLD=${NUAGE_STORM_SCOUT_HOLD:-OFF}/${NUAGE_STORM_MIN_HOLD_SEC:-20}s | STORM_LATCH=${NUAGE_STORM_LATCH:-OFF} | BIDIR_SIDES=ON (AUTO/BOTH + opposite)"
...
ruby -e "sleep ${duration_sec}; File.write('runs/STOP_REASON.txt', %(#{Time.now.utc.strftime('%Y-%m-%dT%H:%M:%SZ')} reason=timer_nominal duration_sec=${duration_sec}\n)); File.write('STOP_ALPHA',''); File.write('STOP_BETA','')" &
PID_TIMER=$!
...
trap 'ace777_process_die_log $?' ERR
trap 'ace777_process_die_log $?' EXIT
