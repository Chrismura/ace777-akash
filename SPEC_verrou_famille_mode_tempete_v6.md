# SPEC v6 — AJUSTEMENTS FINAUX (3 points DEEPSEEK) sur famille_session.py v5

## Statut

v5 validée : JUGE = GO, GEMINI = GO, ULTRA = GO, DEEPSEEK = GO AVEC RÉSERVES.
Le bug du 13/08 est corrigé. Il reste 3 ajustements mineurs (tous de DEEPSEEK) à
appliquer sur le module v5. Le codeur livre la v6 complète (v5 + 3 ajustements).
...
## AJUSTEMENT 1 — Ajouter le déclencheur vortex ≥ 2 dans `mode_tempete_actif()`

Actuellement `mode_tempete_actif()` teste : zone ADA ROUGE/PRENDS_LA_PERTE, alarme
< 1 h, `etat_tempete.json`. Ajouter la lecture de
`STRATEGIE_DIR / "ada_saison_live.json"` → `vortex.force >= 2` :
...
Insérer ce bloc après la vérification de la zone ADA (avant la vérification de
l'alarme), dans le try principal de `mode_tempete_actif()`.

## AJUSTEMENT 2 — TTL créé SEULEMENT si l'occasion existe (pas de faux anti-spam)

Actuellement `consulter_famille()` crée le TTL juste après le flock, PUIS le thread
vérifie `est_une_occasion()`. Si l'occasion est False, le thread retourne tôt mais le
TTL reste → le prochain appel réel (avec occasion) est bloqué 5 min pour rien.

Correction : déplacer la création du TTL dans `_thread_trio()`, APRÈS la vérification
d'occasion et avant le lancement des 3 appels :

- Dans `consulter_famille()` : ne plus appeler `_creer_etat_ttl()` (garder seulement
  le flock). Le flock à lui seul empêche les doublons pendant que le thread tourne.
- Dans `_thread_trio()` : après `occasion, raison = est_une_occasion(live)` et le
  `if not occasion: return`, appeler `_creer_etat_ttl()` (avec le timestamp de début
  réel de la consultation) avant de lancer les 3 threads du trio.
- Conséquence : si l'occasion est False, aucun TTL n'est écrit, aucun faux anti-spam.
  Si l'occasion existe, le TTL est écrit AU DÉBUT réel de la consultation (même
  comportement anti-boucle que v5 : l'anti-spam tient 5 min / 60 s à partir de là).

## AJUSTEMENT 3 — Aligner `join` externe sur la durée du trio

Dans `consulter_famille()`, `t.join(timeout=90)` est plus court que le `join(240)`
interne du trio : en cas de latence, la fonction principale lâche la main alors que
le thread (et le verrou) tourne encore — acceptable mais à aligner. Mettre
`t.join(timeout=245)` (240 + marge) pour que le superviseur attende la fin réelle
dans le cas nominal.

## CONTRAT DE SORTIE v6

Le module `famille_session.py` v6 COMPLET (v5 + ajustements 1-2-3), prêt à copier
dans `Index_Maison/scripts/famille_session.py`. Zéro placeholder, syntaxe valide
Python 3.9, commentaires en français, non fatal.

## FICHIER CONCERNÉ

- `Index_Maison/scripts/famille_session.py` (v6 complète)


---

## ANNEXE — CODE v5 (base, à ajuster)

```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
famille_session.py — Module consultation Famille ACE777 (v5)
Verrou famille + mode tempête + trio réel (Gemini/DeepSeek/Juge)
Fusion v3 (trio réel) + corrections v4 (os.close, mode tempête, TTL après flock) + TTL conservé (v5)
...
def _creer_etat_ttl() -> None:
    """Crée l'état TTL avec timestamp initial (juste après le flock)."""
    try:
        etat = {
            "timestamp": time.time(),
            "source": "famille_session_v5"
        }
...
def _thread_trio(lock_fd: int) -> None:
    ...
    try:
        ...
        try:
            ...
            pass
        # TTL conservé : on note la fin sans modifier le timestamp initial
        try:
...
# ============================================================
# MODE TEMPÊTE (version robuste v4)
# ============================================================
def mode_tempete_actif() -> bool:
    """Détecte le mode tempête selon ADA ROUGE / alarme / etat_tempete."""
    try:
...
def consulter_famille(force: bool = False) -> None:
    ...
    try:
        ...
        # TTL créé immédiatement après le flock réussi (correction v4)
        _creer_etat_ttl()
